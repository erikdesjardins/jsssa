...
pub fn parse(js: &str) -> Result<ast::File, Error> {
    ...
    let babylon_ast = unsafe {
        let runtime = Runtime::new().map_err(|()| SpiderMonkeyError::FailedToInitialize)?;
        ...
        converted_ast = String::from_jsval(context, parse_result.handle(), ())
            .map_err(|()| SpiderMonkeyError::StringWasNull)?;
        converted_ast
            .get_success_value()
            .ok_or(SpiderMonkeyError::FailedToConvertParseResult)?
    };
...
enum SpiderMonkeyError {
    #[fail(display = "SpiderMonkey runtime failed to initialize")]
    FailedToInitialize,
    #[fail(display = "SpiderMonkey failed to convert parse result")]
    FailedToConvertParseResult,
